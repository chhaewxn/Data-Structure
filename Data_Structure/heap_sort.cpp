...
void heap_property(HeapType* h, int parent)
{
	element tmp;
	int max = parent;
	int left = 2 * parent;
	...
	if (h->heap[left].key > h->heap[max].key && left < h->heap_size)
		max = left;
	if (h->heap[right].key > h->heap[max].key && right < h->heap_size)
		max = right;

	if (max != parent) {
		tmp = h->heap[parent];
		h->heap[parent] = h->heap[max];
		h->heap[max] = tmp;
