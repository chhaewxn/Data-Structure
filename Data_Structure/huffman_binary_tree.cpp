...
void huffman_traversal(TreeNode* node)
{
	...
	{
		for (i = 0; i < cnt; i++) {
			m_LUT[(int)node->data - 97][i] = arr[i];
		}
		m_bit_size[(int)node->data - 97] = cnt;
	}
}
...
void huffman_encoding(char* str, bits_stream* bits_str)
{
	...
	for (int i = 0; str[i] != NULL; i++) {
		bits_str->length += m_bit_size[(int)str[i] - 'a'];
		for (int j = 0; j < m_bit_size[(int)str[i] - 'a']; j++)
			bits_str->stream[interval++] = m_LUT[(int)str[i] - 'a'][j];
	}
...
int huffman_decoding(bits_stream* bits_str, TreeNode* node, char* decoded_str)
{
	...
	for (int i = 0; i < bits_str->length; i++)
	{
		...
		{
			decoded_str[index_char] = pos->data;
			pos = node;
			index_char++;
		}
	}
...
void main()
{
	...
	arr = (int*)malloc(sizeof(int) * m_char_size);
